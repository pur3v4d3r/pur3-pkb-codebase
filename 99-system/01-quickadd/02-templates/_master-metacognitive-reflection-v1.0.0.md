<%*
/* METACOGNITIVE REFLECTION TEMPLATE  v1.0.0
   Use weekly/monthly to reflect ON your critical thinking practice itself.
   This is meta-level: not analyzing an argument, but analyzing your analysis. */
const period = await tp.system.suggester(
  ["Daily", "Weekly", "Monthly", "Ad-hoc / Post-session"],
  ["daily",  "weekly", "monthly", "ad-hoc"]
);
if (!period) return;
const fname = `${tp.date.now("YYYY-MM-DD")}_metacog-reflection_${period}`;
await tp.file.rename(fname);
await tp.file.move(`/03-notes/critical-thinking-practice/_reflections/${fname}`);
tR += "";
%>---
title: "Metacognitive Reflection — <% period %> <% tp.date.now('YYYY-MM-DD') %>"
aliases: []
tags:
  - critical-thinking
  - metacognition
...
# Metacognitive Reflection — <% period %>
...
**From:** `INPUT[date:period-start]` **to:** <% tp.date.now("YYYY-MM-DD") %>
