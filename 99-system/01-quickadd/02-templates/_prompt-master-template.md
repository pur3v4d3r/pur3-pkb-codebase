...
// Convention: [prefix]-[semantic-title]-[version]-[timestamp].md
// Example: prompt-text-analysis-1.0.0-20251220143022.md

// 1. Prompt for semantic title
const semanticTitle = await tp.system.prompt("Semantic title (e.g., 'text-analysis', 'code-review'):");
if (!semanticTitle) {
...
// 3. Determine prefix based on prompt type
const prefixOptions = [
...
const prefixChoice = await tp.system.suggester(
    prefixOptions,
    ["prompt", "claude-project", "gemini-gem", "system-prompt", "user-prompt", "workflow", "component"],
    false,
    "Select prompt type/prefix:"
);
...
const filename = `${prefixChoice}-${sanitized}-${version}-${timestamp}`;
...
>
> **Tips**:
> - Use `<% tp.file.cursor() %>` for manual input points
