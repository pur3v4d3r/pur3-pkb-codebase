...
const tempDate = tp.date.now("YYYY-MM-DD-HHmmss");
...
const reviewTime = tp.date.now("HHmmss");
// Rename with actual target note name (include timestamp for uniqueness)
await tp.file.rename(`review-${reviewDate}-${reviewTime}-${targetNote.replace(/[^\w\s-]/g, '').replace(/\s+/g, '-')}`);
