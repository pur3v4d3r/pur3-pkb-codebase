...
A fourth theoretical perspective, developed by [[Patricia Cheng]] and [[Keith Holyoak]] (1985), proposes that much real-world deductive success depends not on formal rules or mental models but on **pragmatic reasoning schemas** — clusters of production rules organized around recurring social situations. The most important schema is the *permission schema*, which is activated when a conditional rule encodes a permission structure ("if action is taken, precondition must be satisfied"). The permission schema guides selection of the logically correct cards in Wason task versions with permission content, even when participants have no formal training in conditional logic.
