---
title: "Dataview Index: for Querying User Definded Values"
id: 20251112-093114
type: ⚙️reference
status: 🌱seedling
rating: ""
source: claude-4.5-sonnet
url: https://claude.ai/chat/7b43dca1-7e19-4eed-9695-5394b043fe21
tags:
  - type/reference
  - type/reference
  - pkb/plugin/dataview
  - source/llm/claude
  - year/2025
aliases:
  - Dataview Query
  - Query Values
  - Dataview
  - Query
  - Automation/Dataview
link-up:
  - "[["
link-related:
date created: 2025-11-12T09:30:37
date modified: 2025-11-12T09:56:11
---

# QUARRIABLE **DATA KEYS**

# QUARRIABLE **DATA VALUES**


## **ALIASES**

```cs
## **ALIASES**
Cognitive Psychology
Educational Psychology
PKB
PKB Infrastructure
Information Architecture
Project Pur3v4d3r

```

## **TYPE**

```cs
## **TYPE**
reference
permanent
composite-prompt

```

## **SOURCE**

```cs
## **SOURCE**

claude-sonnet-4.5
claude-opus-4.1
claude-haiku-4.5
gemini-pro-2.5
gemini-flash-2.5
user/pur3v4d3r/llm/claude
user/pur3v4d3r/llm/gemini

```

## **STATUS**

```cs
## **STATUS**
archived
active
review

```

## **RATING**

```cs
## **RATING**

```

----
# In-Use Metadata-Sets

### Planning Metadata
```
  - type/permanent
  - year/2025
  - project/pur3v4d3r
  - source/report
  - 
  - 
```

```
  - "[[202511160952_academic-report_comprehensive-analysis-of-the-cognitive-mechanisms-underlying-the-intention-action-gap-in-planning]]"
```

```
  - "[[2025-11-16|Daily-Note]]"
  - "[[Permeant-Note_MOC]]"
  - "[[Cognitive Load Management]]"
  - "[[Cognitive Processes]]"
  - "[[Cognitive-Load-Theory]]"
  - "[[Cognitive-Science]]"
  - "[[Distributed-Cognition]]"
  - "[[Educational-Psychology]]"
  - "[[Extended Mind]]"
  - "[[Goal Setting]]"
  - "[[Heutagogy]]"
  - "[[Information Theory]]"
  - "[[Instructional-Design]]"
  - "[[Long-Term-Memory]]"
  - "[[Long Term Working Memory]]"
  - "[[Metacognition]]"
  - "[[Metacognitive-Knowledge]]"
  - "[[Metacognitive-Regulation]]"
  - "[[Motivational Beliefs]]"
  - "[[Motivational Fortitude]]"
  - "[[Planning]]"
  - "[[Pomodoro Technique]]"
  - "[[Private Speech]]"
  - "[[Planning Fallacy]]"
  - "[[Prospective Hindsight]]"
  - "[[Scaffolding]]"
  - "[[Schema]]"
  - "[[Self Behavioral Management]]"
  - "[[Self-Determination-Theory]]"
  - "[[Self-Directed-Learning]]"
  - "[[Self-Regulated-Learning]]"
  - "[[Self Regulation Theory]]"
  - "[[Strategic-Planning]]"
  - "[[System Building]]"
  - "[[Time Blocking]]"
  - "[[Weekly Review]]"
  - "[[Working-Memory]]"
  - "[[Getting Things Done (GTD)]]"
  - "[[Information-Architecture]]"
  - "[[Methodological Frameworks]]"
  - "[[Knowledge-Management]]"
  - "[[Personal-Knowledge-Management]]"
  - "[[Personal-Knowledge-Base]]"
```
### CSS Callout Snippet System Metadata
```
  - year/2025
  - status/seedling
  - topic/pkb/infrastructure
  - type/reference
  - css/obsidian
  - obsidian/snippet/modification
  - snippets
  - plugins/style-settings
  - reference/taxonomy
```
```
  - "[[202511131029_css-snippet-modification-for-obsidian_comprehensive-reference]]"
  - "[[202511131909_complete-modular-callout-system_foundation-and-swappable-styles]]"
  - "[[202511140502_css-snippet-library_comprehensive-reference]]"
  - "[[css-callout-system]]"
  - "[[css-variables-index]]"
  - "[[gemini-callout-snippet-system]]"
  - "[[scratchpad-for-moving-snippets]]"
  - "[[📚obsidian-css-variable-reference]]"
  - "[[📚comprehensive-reference-css-variables]]"
  - "[[snippet-index-note_202511132126]]"
  - "[[202511150329_alternative-checkboxes_reference]]"
```

```
  - Callout CSS Reference
  - Obsidian Callout Styling
  - Callout Customization Guide
  - Callout Customization
  - Callout CSS Taxonomy
  - Obsidian Callout Modification
  - Obsidian Callout CSS
  - Style Settings for Callouts
  - CSS Snippet Reference
  - Callout Theme System
  - Modular Callout System
  - Swappable CSS Snippets
  
```


----



---
