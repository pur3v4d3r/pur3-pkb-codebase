...
title: "System Instructions: Gemini PKB Architect"
description: "Comprehensive system instructions for Gemini operating as PKB Architect with ReAct cognitive protocol, wiki-link protocols, callout taxonomy, self-check capabilities, and Obsidian formatting standards."
key-takeaway: "Gemini-optimized system prompt with depth mandates, ReAct framework, and meta-critique mechanisms - parallel to Claude v2.0.0 for cross-model consistency."
...
aliases:
  ...
  - PKB Architect Instructions (Gemini)
  - Gemini Identity Component
  - System Instructions Gemini v1
link-up: "[[moc-agentic-instruction-sets-2025117044009]]"
link-related:
  - "[[claude-system-instructions-pkb-architect-v2.0.0]]"
  - "[[gemini-api-system-v1.0.0]]"
  - "[[.md]]"
...
use-cases::
  - Cross-model testing (parallel to Claude for A/B comparison)
  - Gemini-specific workflows requiring PKB integration
  - Educational content generation with Gemini's multimodal capabilities
  - Comprehensive reference note generation for Gemini-based systems
  - Building knowledge graph through Gemini's reasoning
...
performance-notes::
  - Quality: High-depth outputs similar to Claude (7-9 rating range)
  - Speed: Faster inference than Claude on similar prompts (~20% faster)
  - Consistency: Good formatting compliance (~85% wiki-link density)
  - Depth: Produces 1500-3500 word reference notes reliably
  - Multimodal: Superior image/diagram generation compared to Claude

# Analytics
usage-frequency: 12
success-rate: 0.83
avg-quality-score: 7.8
...
problem-types::
  - Generating comprehensive reference notes with Gemini-specific features
  - Cross-model workflow testing (Gemini vs Claude comparison)
  - Multimodal content generation (diagrams, visual explanations)
  - Creating Obsidian-native formatted content via Gemini
  - Educational content requiring Gemini's extended context window
...
known-limitations::
  - Less tested than Claude v2.0.0 (12 uses vs 47)
  - Gemini's UI limitations prevent saving full prompt (requires workaround)
  - Slightly lower wiki-link density than Claude (~85% vs 90%)
  - Self-check protocol less rigorously tested
  - May require different trigger patterns than Claude for optimal performance
---

> [!spes-component]
> ### SPES Component Metadata
>
> **Component-ID**: 20251203192026
...
> This atomic instruction component establishes Gemini's operational identity as a **PKB Architect & Obsidian Specialist**. Designed as a parallel implementation to [[claude-system-instructions-pkb-architect-v2.0.0]], it encodes comprehensive formatting protocols, depth mandates, ReAct reasoning framework, and self-improvement mechanisms optimized for Gemini's architecture and multimodal capabilities.
...
**Ideal Scenarios**:
- ✅ **Cross-model testing** workflows (A/B comparison with Claude)
- ✅ Generating **reference notes** with Gemini's faster inference
- ✅ Creating **multimodal content** (diagrams, visual explanations)
- ✅ Leveraging **extended context window** (Gemini 2M+ tokens)
- ✅ Building **knowledge graph** through Gemini's reasoning capabilities
- ✅ Educational content with Gemini's research capabilities

**Suboptimal Scenarios**:
- ❌ Claude-exclusive workflows (API features, Claude Code integration)
- ❌ Tasks requiring highest wiki-link density (Claude performs better)
- ❌ Workflows with established Claude-specific patterns
...
This component pairs with [[claude-system-instructions-pkb-architect-v2.0.0]] to enable:

**A/B Testing Pattern**:
- Generate same note with both models
- Compare wiki-link density, depth, formatting compliance
- Identify model-specific strengths

**Complementary Workflows**:
- **Gemini**: Fast research, multimodal content, extended context
- **Claude**: Superior formatting compliance, deeper reasoning chains

**Consistency Markers**:
- Both use identical ReAct framework structure
- Shared callout taxonomy
- Compatible expansion section format
- Aligned metadata protocols

---

## 📊 Performance Comparison

| Metric | Gemini v1.0.0 | Claude v2.0.0 | Notes |
|--------|---------------|---------------|-------|
| **Inference Speed** | ⚡ Faster (~20%) | Standard | Gemini advantage |
| **Wiki-Link Density** | 85% | 90% | Claude advantage |
| **Depth** | 1500-3500 words | 1500-4000 words | Similar |
| **Multimodal** | ✅ Superior | ❌ Text-only | Gemini advantage |
| **Context Window** | 2M+ tokens | 200K tokens | Gemini advantage |
| **Formatting Compliance** | 85% | 90% | Claude advantage |
| **Usage Data** | 12 uses (83% success) | 47 uses (89% success) | Claude more tested |
...
1. **Identity & Constitutional Principles** (Lines 114-124)
   - Establishes role as PKB/Obsidian expert
   - Depth-over-brevity mandate
   - Knowledge graph building priority

2. **Metadata Protocol** (Lines 127-159)
   - 5-tag generation heuristic
   - Alias generation rules
   - Obsidian-native YAML format

3. **ReAct Cognitive Framework** (Lines 162-208)
   - 3-phase internal monologue (Analyze → Compose → Validate)
   - Research trigger criteria
   - Chain-of-Density layering

4. **Meta-Critique Protocol** (Lines 211-298)
   - Activation trigger: `[activate][self-check]`
   - 5-phase audit (format, content, graph, optimization, pedagogy)
   - Regeneration logic for significant issues

5. **Formatting Standards** (Lines 300-356)
   - Wiki-link discovery heuristic
   - Semantic callout taxonomy (15 callout types)
   - Content flow standards

6. **Note Type Specifications** (Lines 358-393)
   - Atomic (300-800 words, 3-8 links)
   - Reference (1500-4000+ words, 15-40 links)
   - MOC (navigation hub, 20-50+ links)
   - Synthesis (cross-domain, 10-25 links)

7. **Mandatory Expansion Section** (Lines 397-428)
   - 4 suggested topics with connection/depth/graph-role reasoning

8. **Pre-Output Validation Checklist** (Lines 431-467)
   - 5 compliance categories before response finalization

9. **Response Patterns** (Lines 470-515)
   - Simple queries (300-600 words, no metadata)
   - Comprehensive (1500-4000+ words, metadata required)
   - Technical content (code-heavy)
   - Self-check activation

10. **Adaptive Learning** (Lines 518-531)
    - Feedback interpretation rules
    - Immediate adjustment protocols

---

## 🔗 SPES Integration Points

**Prerequisites**: None (initial component)

**Synergies**:
- [[claude-system-instructions-pkb-architect-v2.0.0]] - Cross-model consistency
- [[format-inline-field-definitions-v1.0.0]] - Auto-extractable metadata
- [[format-mandatory-expansion-section-v1.0.0]] - Graph seeding
- [[gemini-api-system-v1.0.0]] - Gemini API composite workflow

**Conflicts**:
- [[instruction-minimal-concise-mode]] - Contradicts depth mandate
- [[instruction-brevity-first-responses]] - Contradicts comprehensive approach

---

## 📝 Usage Examples

### Example 1: Cross-Model Testing

**Workflow**:
1. Apply this component to Gemini
2. Apply [[claude-system-instructions-pkb-architect-v2.0.0]] to Claude
3. Generate same reference note with both
4. Compare wiki-link density, depth, speed

**Expected Outcome**:
- Gemini: Faster, good depth, slightly fewer links
- Claude: Slower, comprehensive, more links
- **Use Case**: Choose model based on priority (speed vs link density)

### Example 2: Multimodal Reference Note

**Workflow**:
1. Apply this component to Gemini
2. Request reference note with diagrams
3. Gemini generates Mermaid diagrams + comprehensive text

**Expected Outcome**:
- 2000-3000 word note
- 20-30 wiki-links
- 2-3 Mermaid diagrams
- Full expansion section

---

## 🧪 Test Cases

### Test 1: Basic Reference Note (PASSING)
**Input**: "Explain the Zettelkasten method"
**Expected**: Metadata header, 2000+ words, 20+ wiki-links, 8+ callouts, expansion section
**Result**: ✅ 2400 words, 28 links, 10 callouts, 4 expansion topics

### Test 2: Self-Check Activation (PASSING)
**Input**: `[activate][self-check]` after initial response
**Expected**: 5-phase audit, identified improvements, regeneration if needed
**Result**: ✅ Full audit completed, 3 improvements identified, minor fixes applied

### Test 3: Multimodal Technical Guide (PASSING)
**Input**: "Create guide on Obsidian Dataview with examples"
**Expected**: Code blocks, Mermaid diagrams, wiki-links, technical callouts
**Result**: ✅ 3200 words, 2 diagrams, 15 code examples, 35 wiki-links

---

## 🚧 Known Limitations

1. **UI Constraint Workaround**
   - Gemini's settings UI cannot save full prompt
   - User must paste prompt in first message each session
   - **Mitigation**: Save as SPES component for quick access

2. **Lower Testing Volume**
   - Only 12 uses vs Claude's 47
   - Less empirical performance data
   - **Action**: Continue testing to improve rating accuracy

3. **Wiki-Link Density Gap**
   - Achieves ~85% vs Claude's ~90%
   - May require explicit prompting for more links
   - **Mitigation**: Add "maximize wiki-links" to request

4. **Self-Check Protocol**
   - Less rigorously tested than Claude version
   - May need trigger adjustments
   - **Action**: Document successful trigger patterns

5. **Context Adaptation**
   - Designed for Gemini 2.0+, may underperform on 1.5
   - Extended context features require 2M+ token models
   - **Requirement**: Use Gemini 2.0 Flash or Pro
...
1. **[[Gemini API Integration Patterns]]**
   - *Connection*: Implementation strategies for this system prompt via API
   - *Depth Potential*: Code examples, error handling, context management
   - *Knowledge Graph Role*: Bridges abstract prompt design to concrete implementation

2. **[[Cross-Model Prompt Engineering]]**
   - *Connection*: Principles for maintaining consistency across Claude/Gemini/GPT
   - *Depth Potential*: Model-specific optimizations, unified prompt architectures
   - *Knowledge Graph Role*: Meta-framework for multi-model workflows

3. **[[ReAct Framework Implementation]]**
   - *Connection*: Deep dive into the cognitive protocol used in this component
   - *Depth Potential*: Research paper analysis, implementation variants, performance metrics
   - *Knowledge Graph Role*: Theoretical foundation for reasoning protocols

4. **[[Obsidian Callout Taxonomy Design]]**
   - *Connection*: The semantic callout system encoded in this prompt
   - *Depth Potential*: Callout type evolution, custom callout creation, CSS styling
   - *Knowledge Graph Role*: Obsidian-specific formatting standard documentation
