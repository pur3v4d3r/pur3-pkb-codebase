---
title: john-flavell
aliases:
- john-flavell
type: permanent-note
status: enriched
confidence: low
tags:
- permanent-note
- seedling
- concept-stub
- other
domain: cognitive-psychology
created: 2026-04-22
updated: '2026-04-22'
source-type: stub-generation
extraction-method: generate-stubs-v1 (auto-generated from wiki-link audit)
referenced-by-count: 185
see-also:
- '[[AI-Assisted-Reading-Risks-and-Safeguards]]'
- '[[Activating-Prior-Knowledge-for-This-Report|Activating Prior Knowledge for This
  Report]]'
- '[[Application-Desirable-Difficulties-and-Monitoring|Application Desirable Difficulties
  and Monitoring]]'
- '[[Application-Why-Self-Explanation-Works|Application Why Self-Explanation Works]]'
- '[[Artificial-Intelligence-Self-Monitoring-Systems|Artificial Intelligence Self-Monitoring
  Systems]]'
- '[[Asher-Koriat-b.-1942|Asher Koriat (b. 1942)]]'
- '[[Athletic-Skill-Acquisition-and-Motor-Learning|Athletic Skill Acquisition and
  Motor Learning]]'
- '[[Bridging-to-Prior-Knowledge-The-PKB-as-Cognitive-Partner|Bridging to Prior Knowledge
  The PKB as Cognitive Partner]]'
- '[[CLT-as-the-Unifying-Diagnostic-for-PKB-Design-Failures|CLT as the Unifying Diagnostic
  for PKB Design Failures]]'
- '[[Calibration-Metacognitive-Accuracy-Literature|Calibration (Metacognitive Accuracy
  Literature)]]'
review-frequency: quarterly
mastery-stage: budding
importance: high
maturity: budding
provenance:
  enrichment-method: enrich_stubs-v1
  enrichment-model: qwen2.5:7b-instruct-q5_K_M
parent-moc:
- '[[metacognition-moc]]'
- '[[researchers-and-theorists-moc]]'
---

# john-flavell

> [!definition] john-flavell
> - **Key-Term**: [[john-flavell]]
> - **Definition**: John Flavell is an influential cognitive psychologist known for his work on metacognition, particularly the development of self-knowledge and self-regulation in children and adults.
> - **Domain**: other
> - **Status**: 🌱 budding | Confidence: speculative

## Core Explanation

> [!analytical-insight] Core Explanation
> Flavell's research focuses on how individuals monitor their own thinking processes (metacognition) and adjust them accordingly. He introduced the concept of 'metacognitive knowledge' which includes awareness of one’s own cognitive processes, strategies for problem-solving, and beliefs about learning.

> [!analytical-insight] Explanation 2
> In practice, Flavell’s theories have been applied in educational settings to enhance teaching methods that promote self-regulated learning, such as encouraging students to engage in self-explanation and reflection on their thought processes.

> [!analytical-insight] Explanation 3
> Key nuances include the distinction between procedural and declarative metacognitive knowledge, where procedural knowledge involves knowing how to do something, while declarative knowledge involves knowing what one knows.

## Practical Implications

> [!example] Application
> In education, Flavell’s theories have led to the development of instructional strategies that encourage students to think about their thinking (metacognition) and use self-explanation techniques to deepen understanding.

> [!example] Application
> In artificial intelligence, his work on metacognition has inspired AI systems designed to monitor and adjust their own processes for more effective problem-solving.

## Key Figures

> [!person] John Flavell
> cognitive psychologist

## Connections

**Related:** [[Metacognition]] · [[Self-Regulated Learning]] · [[Cognitive Development]]

**See Also (existing):**
- [[AI-Assisted-Reading-Risks-and-Safeguards]]
- [[Activating-Prior-Knowledge-for-This-Report|Activating Prior Knowledge for This Report]]
- [[Application-Desirable-Difficulties-and-Monitoring|Application Desirable Difficulties and Monitoring]]
- [[Application-Why-Self-Explanation-Works|Application Why Self-Explanation Works]]
- [[Artificial-Intelligence-Self-Monitoring-Systems|Artificial Intelligence Self-Monitoring Systems]]
- [[Asher-Koriat-b.-1942|Asher Koriat (b. 1942)]]
- [[Athletic-Skill-Acquisition-and-Motor-Learning|Athletic Skill Acquisition and Motor Learning]]
- [[Bridging-to-Prior-Knowledge-The-PKB-as-Cognitive-Partner|Bridging to Prior Knowledge The PKB as Cognitive Partner]]

```dataview
LIST FROM [[john-flavell]]
WHERE file.path != this.file.path
SORT file.mtime DESC
LIMIT 10
```

---

**Sources:** *(auto-enriched from domain knowledge)*