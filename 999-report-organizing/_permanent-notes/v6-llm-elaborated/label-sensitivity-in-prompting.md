...
Label sensitivity is exacerbated in contexts where labels serve as both cues and responses, such as in classification tasks. When models learn to associate certain label tokens with specific inputs, they may develop a preference for these tokens over semantically equivalent alternatives. This phenomenon can be understood through the lens of top-down vs bottom-up processing: frequent exposure to particular label strings (top-down) shapes model expectations more strongly than the raw input data alone (bottom-up).
...
> In Massive Open Online Courses (MOOCs), spaced retrieval techniques are used to enhance learning retention. However, if label sensitivity is not addressed, models might perform poorly on assessments that use less common labels for correct answers. For instance, a model trained primarily with 'correct' and 'incorrect' labels may struggle when asked to identify 'right' or 'wrong'. This could lead to misleading feedback loops where the system reinforces less effective labeling practices.
...
> [!key-distinction] **Recognition vs Recall in Label Sensitivity**
> In recognition tasks, models are provided with a set of options and must choose the correct label from among them. In contrast, recall tasks require generating labels without such prompts. Recognition can mask label sensitivity because the presence of familiar labels aids performance. However, recall tasks expose this bias more clearly as models struggle to produce less common labels. Understanding these differences is crucial for designing fair evaluation metrics in few-shot learning.

## Common Misconceptions

<!-- enhancement-pass:1 (2026-05-23) -->

> [!warning] **Misconception** — Label sensitivity only affects model accuracy.
>
> While label sensitivity can indeed impact accuracy, its effects extend beyond this to influence the reliability and fairness of models. For instance, a model biased towards certain labels may consistently misclassify inputs that are semantically similar but labeled differently in training data. This not only reduces overall performance but also introduces biases that could perpetuate unfair outcomes.
...
<!-- enhancement-pass:1 (2026-05-23) -->
Addressing label sensitivity requires a multi-faceted approach that includes not only technical solutions like calibration techniques but also a deep understanding of how models process and respond to different types of labels. By integrating insights from cognitive psychology, such as the recognition vs recall distinction, researchers can develop more robust and fair few-shot learning systems.
